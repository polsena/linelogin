<?php
/**
 * Plugin Name: LINE + Gmail Login Integration
 * Description: Adds LINE Login and Google (Gmail) Login integration to WordPress.
 * Version: 1.2.0
 * Author: ChatGPT
 */
...
class Line_Login_Plugin {
    ...
    const META_LINE_USER_ID = 'line_user_id';
    const META_GOOGLE_SUB = 'google_sub';

    const STATE_TRANSIENT_PREFIX = 'line_login_state_';

    public function __construct() {
        add_action('login_form', [ $this, 'add_login_buttons' ]);
        add_action('init', [ $this, 'handle_oauth_requests' ]);
        add_action('admin_menu', [ $this, 'add_settings_page' ]);
        add_action('admin_init', [ $this, 'register_settings' ]);
    }

    public function add_login_buttons() {
        echo '<p style="text-align:center;"><a class="button button-primary button-large" href="' . esc_url(home_url('/?line-login=1')) . '">' . esc_html__('Login with LINE', 'line-login') . '</a></p>';
        echo '<p style="text-align:center;"><a class="button button-secondary button-large" href="' . esc_url(home_url('/?google-login=1')) . '">' . esc_html__('Login with Google (Gmail)', 'line-login') . '</a></p>';
    }
    ...
    public function handle_oauth_requests() {
        ...
        if (isset($_GET['google-login-callback'])) {
    ...
    protected function verify_oauth_state($state, $provider) {
        ...
        $saved_provider = get_transient($key);
    ...
    protected function redirect_to_line() {
        $channel_id = trim((string) get_option(self::OPTION_LINE_CHANNEL_ID, ''));
    ...
    protected function process_line_callback() {
        ...
        if ($code === '' || $state === '' || !$this->verify_oauth_state($state, 'line')) {
        ...
        $user = $this->find_user_by_meta(self::META_LINE_USER_ID, $line_id);

        if (!$user) {
            $user = $this->login_or_create_user('line_' . $line_id, '', $display_name);
        }

        update_user_meta($user->ID, self::META_LINE_USER_ID, $line_id);
        $this->complete_login($user);
    }

    protected function redirect_to_google() {
        $client_id = trim((string) get_option(self::OPTION_GOOGLE_CLIENT_ID, ''));
    ...
    protected function process_google_callback() {
        ...
        if ($code === '' || $state === '' || !$this->verify_oauth_state($state, 'google')) {
        ...
        $user = $this->find_user_by_meta(self::META_GOOGLE_SUB, $google_sub);
        ...
        if (!$user) {
            $preferred_username = $email !== '' ? $email : 'google_' . $google_sub;
            $user = $this->login_or_create_user($preferred_username, $email, $display_name);
        }
    ...
    protected function find_user_by_meta($meta_key, $meta_value) {
        $users = get_users([
            'number'     => 1,
            'meta_key'   => $meta_key,
    ...
    protected function login_or_create_user($preferred_login, $email, $display_name) {
        $user = false;
    ...
    public function add_settings_page() {
        add_options_page('LINE + Gmail Login', 'LINE + Gmail Login', 'manage_options', 'line-login', [ $this, 'render_settings' ]);
    }
    ...
    public function register_settings() {
        ...
        add_settings_section('google_settings', 'Google / Gmail Settings', '__return_false', 'line-login');
    ...
    public function render_settings() {
        echo '<div class="wrap"><h1>LINE + Gmail Login</h1><form method="post" action="options.php">';
    ...
    public function field_line_channel_id() {
        $value = esc_attr((string) get_option(self::OPTION_LINE_CHANNEL_ID, ''));
        echo '<input type="text" name="' . esc_attr(self::OPTION_LINE_CHANNEL_ID) . '" value="' . $value . '" class="regular-text" />';
    }

    public function field_line_channel_secret() {
        $value = esc_attr((string) get_option(self::OPTION_LINE_CHANNEL_SECRET, ''));
        echo '<input type="password" name="' . esc_attr(self::OPTION_LINE_CHANNEL_SECRET) . '" value="' . $value . '" class="regular-text" autocomplete="off" />';
    }

    public function field_google_client_id() {
        $value = esc_attr((string) get_option(self::OPTION_GOOGLE_CLIENT_ID, ''));
        echo '<input type="text" name="' . esc_attr(self::OPTION_GOOGLE_CLIENT_ID) . '" value="' . $value . '" class="regular-text" />';
    }

    public function field_google_client_secret() {
        $value = esc_attr((string) get_option(self::OPTION_GOOGLE_CLIENT_SECRET, ''));
        echo '<input type="password" name="' . esc_attr(self::OPTION_GOOGLE_CLIENT_SECRET) . '" value="' . $value . '" class="regular-text" autocomplete="off" />';
    }
}
